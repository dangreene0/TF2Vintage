...
void CScriptGameEventListener::LoadEventsFromFile( const char *filename, const char *pathID )
{
	...
	Msg( "CScriptGameEventListener::LoadEventsFromFile: Loaded [%s]%s (%i)\n", pathID, filename, count );
}
#endif
...
{
	m_MsgIn.StartReading( msg->m_pData, msg->m_nDataBytes, msg->m_iCurBit );
#else
void CNetMsgScriptHelper::ReceiveMessage( bf_read &msg )
{
	...
	if ( !g_pScriptVM )
	{
		Warning( DLL_LOC_STR " CNetMsgScriptHelper: No VM on receiving side\n" );
		return;
	}
...
const Vector& CNetMsgScriptHelper::ReadVec3Normal()
{
	static Vector vec3;
	//vec3.Init();
	m_MsgIn.ReadBitVec3Normal(vec3);
	return vec3;
}
...
{
	static char buf[512];
	m_MsgIn.ReadString( buf, sizeof(buf) );
	return buf;
}

bool CNetMsgScriptHelper::ReadBool()
{
	return m_MsgIn.ReadOneBit();
}

HSCRIPT CNetMsgScriptHelper::ReadEntity()
{
	int index = m_MsgIn.ReadSBitLong( MAX_EDICT_BITS );
#ifdef GAME_DLL
...
{
	int iEncodedEHandle = m_MsgIn.ReadLong();
